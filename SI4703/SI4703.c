...
{			
	...
	/* Enable Oscillator  */
	SI4703_Regs[REG_TEST1] = 0x8000;		/* AN230 page 12, why 0x8100 ??? */
	...
	/* Power-up sequence */
	SI4703_Regs[REG_POWERCFG] |= _BV(IDX_DMUTE);
	SI4703_Regs[REG_POWERCFG] |= _BV(IDX_ENABLE);
	SI4703_Regs[REG_POWERCFG] |= _BV(IDX_DISABLE);
	
	/* Set Seek Mode as Stop at band limit */
	SI4703_Regs[REG_POWERCFG] |= _BV(IDX_SKMODE);
	
	/* Enable RDS */
	SI4703_Regs[REG_SYSCONFIG1] |= _BV(IDX_RDS);
	
	/* Set De-Emphasis 75us (Korea) */
	SI4703_Regs[REG_SYSCONFIG1] &= ~_BV(IDX_DE);
	
	/* Set Band as 00 (Korea) */
	SI4703_Regs[REG_SYSCONFIG2] &= ~(_BV(IDX_BAND0)|_BV(IDX_BAND1));
	
	/* Set Space as 00 (Korea) */
	SI4703_Regs[REG_SYSCONFIG2] &= ~(_BV(IDX_SPACE0)|_BV(IDX_SPACE1));
			
	/* Set Volume as 1 */
	SI4703_Regs[REG_SYSCONFIG2] &= 0xFFF0;
	SI4703_Regs[REG_SYSCONFIG2] |= 0x01;
...
bool SI4703_SetMono(bool mono)
{
	...
	if(mono)
	{
		SI4703_Regs[REG_POWERCFG] |= _BV(IDX_MONO);		
	}
	else
	{
		SI4703_Regs[REG_POWERCFG] &= ~_BV(IDX_MONO);
...
bool SI4703_SetFreq(float freq)
{
	...
	SI4703_Regs[REG_CHANNEL] |= _BV(IDX_TUNE);
...
bool SI4703_SeekUp()
{
	...
	/* Set SEEKUP + SEEK bit */
	SI4703_Regs[REG_POWERCFG] |= _BV(IDX_SEEKUP);
	SI4703_Regs[REG_POWERCFG] |= _BV(IDX_SEEK);
...
bool SI4703_UpdateRadioInfo()
{
	...
	radioInfo.ReadChannel = (SI4703_Regs[REG_READCHAN] & MASK_READCHAN) >> 14;
...
static bool SI4703_RxRegs()
{
	...
	for(int i=0; i < 16; i++)
	{
		SI4703_Regs[i] |= (buffer[2*i] << 8);
		SI4703_Regs[i] |= buffer[2*i+1];
	}	
...
static void SI4703_Reset(void)
{
	SI4703_PORT &= ~_BV(SI4703_RST);
	_delay_ms(1);							/* Min 100us */
	SI4703_PORT |= _BV(SI4703_RST);
	_delay_ms(1);							/* Min 100us */
}
